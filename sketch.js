...
if (isOn==true){
  ...
  if (hole_new == 50000000000000 && line_yellow_data_v1.length>0) {
    ...
    for (let i =0; i<136;i++) {
      let y1 = (current_array[i+1]-current_array[i])/0.2*Math.pow(10,7)
    ...
    if (V_applied_p>0) {
      ...
      let y = E_field_temp_data[i].y/Math.pow(10,5)+(10+385/2+96.25) * s_y
      vertex(x, y);
      }
     //  vertex(charge_density_temp_data[136].x, charge_density_temp_data[136].y)
...
beginShape();
 ...
 if (hole_new == 50000000000000){
  curveVertex((250+10*k/2)*s_x,(171.25+current_array[k]*40-100)*s_y)
      ...
      line_yellow_data_v1[k] = { x: (250+10*k/2)*s_x, y: (171.25+current_array[k]*40-100)*s_y};
...
beginShape();
  ...
  for (var k = 0; k < 137; k++) {
    ...
    if (hole_new == 50000000000000){
      curveVertex((250+10*k/2)*s_x,(-30+171.25+current_array[k]*40-30)*s_y)

    }
      line_green_data_v1[k] = { x: (250+10*k/2)*s_x, y: (171.25+current_array[k]*40-30-30)*s_y};
